import { faGear, faEyeDropper, faDownload, faScrewdriverWrench } from "@fortawesome/free-solid-svg-icons";
...
export default function ProjectSettings() {
    ...
    const handleClickOutside = (event: MouseEvent) => {
        if (dialogRef.current && dialogRef.current.open && event.target instanceof Element && !event.target.closest('.modal')) {
            ...
            >
                <div className="modal-content bg-white h-25 w-45 rounded flex flex-col p-2! justify-between">
                    {Object.entries(Settings).map(([name, {icon, color}]) => (
                        <ProjectSettingButton key={name} icon={icon} color={color}>{name}</ProjectSettingButton>
                    ))}
                </div>
            </dialog>
        </div>
...
function ProjectSettingButton({color, icon, children}: {color: string, icon: IconProp, children: React.ReactNode}) {
    return (
        <Button 
            icon={icon}
            iconColor={color}
