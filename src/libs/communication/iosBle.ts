...
export class IOSBluetoothCommunication implements Communication {
    ...
    async write(messages: string | string[]): Promise<void> {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            this.parent.handleMessage({ type: 'error', message });
            throw new Error('Could not write to Ro/Box!', { cause: error as any });
        }
    }
    ...
    private async chunkedWrite(message: string): Promise<void> {
        ...
        for (let i = 0; i < bytes.length; i += CHUNK_SIZE) {
            ...
            console.log("Writing chunk to Ro/Box:", { chunk: slice, chunkString: this.decoder.decode(slice) });
            await BleClient.write(this.deviceId!, numberToUUID(UART_SERVICE), numberToUUID(UART_CHARACTERISTIC), dataView);
    ...
    async disconnect(): Promise<void> {
        try {
            ...
            await BleClient.stopNotifications(this.deviceId, numberToUUID(UART_SERVICE), numberToUUID(UART_CHARACTERISTIC));
            ...
            throw new Error(error instanceof Error ? error.message : String(error || 'Could not disconnect from Ro/Box'));
        }
    }
